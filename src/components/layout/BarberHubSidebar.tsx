...
export function BarberHubSidebar() {
  ...
  return (
    <>
      ...
      >
        {isOpen ? <ChevronLeft className="w-5 h-5" /> : <Scissors className="w-5 h-5" />}
        {!isOpen && <span className="text-sm font-semibold">Barber Hub</span>}
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.aside
            ...
            className="fixed top-0 left-0 h-screen w-64 bg-card border-r border-border z-50 flex flex-col"
          >
            {/* Header */}
            <div className="p-6 border-b border-border">
              <Link to="/barber-hub" className="flex items-center gap-3" onClick={() => setIsOpen(false)}>
